...
> Совместимо с **Remnawave 3.x** — от 3.2.1 до 3.4.3.
>
...
- Работающая панель Remnawave **3.x** (3.2.1 – 3.4.3). Более старые версии не поддерживаются: в 3.0.0 сменился идентификатор пользователя и часть эндпоинтов
- Токен Telegram-бота
...
Обновляете панель — положите её OpenAPI-спеку в `docs/`, поправьте путь
в тесте и прогоните `python -m pytest`. Спека генерируется из исходников панели:

```bash
git clone https://github.com/remnawave/backend.git && cd backend
git checkout <тег версии>       # например 3.4.3
npm ci
BUILD_GEN_DOC=1 npx rspack build --mode development && node dist/gen-doc.js
# результат — openapi.json в корне репозитория
```

> На Windows запускайте две последние команды в bash напрямую, а не через
> `npm run generate:openapi`: npm отдаёт скрипт в `cmd.exe`, где присваивание
> `BUILD_GEN_DOC=1` перед командой не работает и сборка молча собирается без
> генератора.
